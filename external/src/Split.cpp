...
void Split::generalSplit(const vector<int>& chromosome, Individual* indiv, int nbMaxVehicles)
{
	// Do not apply Split with fewer vehicles than the trivial (LP) bin packing bound
	maxVehicles = std::max<int>(nbMaxVehicles, static_cast<int>(std::ceil(instance->total_demand_/instance->max_vehicle_capa_)));
	...
	{
		cliSplit[i].demand = instance->customers_[chromosome[i - 1]].demand;
        cliSplit[i].serviceTime = instance->customers_[chromosome[i - 1]].service_duration;
        cliSplit[i].d0_x = instance->distances_[0][chromosome[i - 1]];
        cliSplit[i].dx_0 = instance->distances_[chromosome[i - 1]][0];
        cliSplit[i].dnext = (i < instance->num_customer_) ? instance->distances_[chromosome[i - 1]][chromosome[i]] : -1.e30;
	...
	if (splitSimple(chromosome, indiv) == 0)
		splitLF(chromosome, indiv);

	// Build up the rest of the Individual structure
//	indiv->evaluateCompleteCost();
}

int Split::splitSimple(const vector<int>& chromosome, Individual* indiv)
{
	...
	if (instance->is_duration_constraint_)
	{
		for (int i = 0; i < instance->num_customer_; i++)
		{
			...
			for (int j = i + 1; j <= instance->num_customer_ && load <= 1.5 * instance->max_vehicle_capa_; j++)
			{
				...
				double cost = distance + cliSplit[j].dx_0
					+ instance->penalty_capacity_ * std::max<double>(load - instance->max_vehicle_capa_, 0.)
					+ instance->penalty_duration_ * std::max<double>(distance + cliSplit[j].dx_0 + serviceDuration - instance->duration_limit_, 0.);
    ...
    int count_routes = 0;
    int temp_end = instance->num_customer_;
    // Traverse backwards, counting unique route breaks
    while (temp_end != 0) {
        temp_end = pred[0][temp_end];  // Move to the predecessor
        count_routes++;      // Count each transition
    }

    indiv->num_routes = count_routes;
    int end = instance->num_customer_;
    for (int i = count_routes - 1; i >= 0; i--) {
        int begin = pred[0][end];
        int customer_pos = 1;
        for (int ii = begin; ii < end; ii++)
            indiv->routes[i][customer_pos++] = chromosome[ii];
        indiv->num_nodes_per_route[i] = customer_pos + 1;
...
//
//        int customer_pos = 1;
//        for (auto it = chromosome.begin() + begin; it < chromosome.begin() + end; ++it) {
//            indiv->routes[route_index][customer_pos++] = *it;
//        }
//        indiv->num_nodes_per_route[route_index] = customer_pos + 1;
//
...
//        }
//    }
//    indiv->num_routes = route_index;

    return 1;
}

// Split for problems with limited fleet
int Split::splitLF(const vector<int>& chromosome, Individual* indiv)
{
...
//	for (int k = instance->num_vehicle_-1; k >= nbRoutes ; k--)
//		indiv->chromR[k].clear();
	...
	for (int k = nbRoutes - 1; k >= 0; k--)
	{
		...
		int customer_pos = 1;
        for (int ii = begin; ii < end; ii++)
            indiv->routes[k][customer_pos++] = chromosome[ii];
		end = begin;
        indiv->num_nodes_per_route[k] = customer_pos + 1;
	}
    indiv->num_routes = indiv->route_cap;

    // Return OK in case the Split algorithm reached the beginning of the routes
	return (end == 0);
}

Split::Split(Case* instance): instance(instance)
{
	...
	potential = std::vector<vector<double>>(instance->num_vehicle_ + 1, std::vector<double>(instance->num_customer_ + 1,1.e30));
	pred = std::vector < std::vector <int> >(instance->num_vehicle_ + 1, std::vector<int>(instance->num_customer_ + 1, 0));
}
